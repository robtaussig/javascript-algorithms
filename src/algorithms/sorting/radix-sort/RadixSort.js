...
export default class RadixSort extends Sort {
  ...
  placeElementsInCharacterBuckets(array, index, numPasses) {
    ...
    const buckets = this.createBuckets(26);
  ...
  placeElementsInNumberBuckets(array, index) {
    // See below. modded and divided are used to determine which digit to use for bucket allocation
    const modded = 10 ** (index + 1);
    const divided = 10 ** index;
    const buckets = this.createBuckets(10);
